...
export async function GET(request: NextRequest) {
  ...
  return withAdminApiAuth(request, async () => {
    ...
    try {
      ...
      logStructured("info", "jobs_list", { requestId, route, durationMs, count: jobs.length });
      return NextResponse.json({ jobs, nextCursor });
    } catch (error) {
      const durationMs = Date.now() - startedAt;
      logStructured("error", "jobs_list_failed", { requestId, route, durationMs });
...
export async function POST(request: NextRequest) {
  ...
  return withAdminApiAuth(request, async () => {
    ...
    if (existing) {
      ...
      logStructured("warn", "manual_job_conflict", {
        requestId,
        route,
    ...
    try {
      ...
      logStructured("info", "manual_job_created", {
        requestId,
        route,
        durationMs: Date.now() - startedAt,
        jobId: job.id,
      ...
      await recordJobAuditEvent({
        ...
        actor: "admin",
        details: { source: "manual_job_form" },
      });
      return NextResponse.json({ job }, { status: 201 });
    } catch {
      logStructured("error", "manual_job_create_failed", {
        requestId,
        route,
        durationMs: Date.now() - startedAt,
        jobId: id,
